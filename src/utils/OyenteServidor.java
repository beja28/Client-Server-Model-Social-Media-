...
import servidor.ServerEstadoConsola;
...
/*
	...
	3.- Entonces se crea una instancia de ConexionCliente pasandole el socket. Esta clase se encarga de la comunicación con el cliente
...
public class OyenteServidor implements Runnable, Closeable {
	 ...
	 * 
	 * Esta interfaz esta diseñada para realizar una operacion que acepta un solo argumento, en concreto el "socket"
	 * 
	 * Cuando se llama a la funcion .accept(socket) realiza la operación que se define en la funcion lambda
	...
	public void run() {
		try {
			...
			ServerEstadoConsola.log("ERROR", "El puerto " + puerto + " ya está en uso por otra aplicacion");
		    throw new RuntimeException(e);
    ...
    public void close() {
        if (this.serverSocket != null) {
            try {
                ...
                throw new UncheckedIOException("Error al cerrar el ServerSocket", ex);
            }
        }
    }
}
